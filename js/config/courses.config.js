/* ==========================================================================
   ...
   Field reference:
     ...
     tags          short topic pills shown on the card (2-4 words each)
...
(function (window) {
  ...
  var COMBO_DISCOUNT = 0.20; // 20% off when a visitor buys every paid course at once
  ...
  var COURSES = [
    {
      ...
      isFree: true,
      price: 0,
      tags: ['9 chương', 'Theo SGK', '3 cấp độ'],
      highlights: [
    ...
    },
    {
      ...
      isFree: false,
      price: 199000,
      tags: ['30 câu/vòng', 'Có tính giờ', 'Streak thưởng'],
      highlights: [
    ...
    },
    {
      ...
      isFree: false,
      price: 149000,
      tags: ['A / An / The', '2 chế độ', 'Huy chương'],
      highlights: [
    ...
    },
    {
      ...
      shortDesc: 'Luyện đủ 4 kỹ năng Nghe – Đọc – Viết – Nói theo chuẩn IELTS, có nhận diện giọng nói và tuỳ chỉnh theo band điểm mục tiêu.',
      description: 'Luyện thi IELTS toàn diện với 4 khu vực riêng biệt cho Listening, Reading, Writing và Speaking. Chọn band điểm mục tiêu (5.0 đến 8.0+) để nội dung tự điều chỉnh độ khó, luyện nghe bằng giọng đọc tổng hợp có thể chỉnh tốc độ và giọng nam/nữ, và luyện nói với nhận diện giọng nói ngay trên trình duyệt.',
      subject: 'IELTS',
      level: 'Band 5.0 – 8.0+',
      accentColor: '#38bdf8',
      accentDeep: '#2563eb',
      thumbnail: 'assets/images/ielts-master.svg',
      isFree: false,
      price: 349000,
      tags: ['4 kỹ năng', 'AI chấm điểm', 'Band 5–8+'],
      highlights: [
        'Đủ 4 kỹ năng: Nghe, Đọc, Viết, Nói',
        'Luyện nói với nhận diện giọng nói ngay trên trình duyệt',
        'Tuỳ chỉnh độ khó theo band điểm mục tiêu (5.0–8.0+)'
      ],
      lessonPath: 'lessons/ielts_practice_v4.html'
    }
  ];
  ...
  /**
   * Computes "buy every paid course at once" pricing dynamically from
   * COURSES, so it stays correct automatically if a course/price changes —
   * nothing about the combo is hard-coded.
   * @returns {{courses:object[], courseIds:string[], originalTotal:number, discountedTotal:number, savings:number, discountPercent:number}}
   */
  function getComboPricing() {
    var paid = COURSES.filter(function (c) { return !c.isFree; });
    var originalTotal = paid.reduce(function (sum, c) { return sum + c.price; }, 0);
    var discountedTotal = Math.round(originalTotal * (1 - COMBO_DISCOUNT));
    return {
      courses: paid,
      courseIds: paid.map(function (c) { return c.id; }),
      originalTotal: originalTotal,
      discountedTotal: discountedTotal,
      savings: originalTotal - discountedTotal,
      discountPercent: Math.round(COMBO_DISCOUNT * 100)
    };
  }

  HV.COURSES = COURSES;
  HV.getCourse = getCourse;
  HV.COMBO_DISCOUNT = COMBO_DISCOUNT;
  HV.getComboPricing = getComboPricing;
