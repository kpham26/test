...
(function (window) {
  ...
  var ADMIN_EMAIL = 'admin@hoclab.local';
  ...
  function publicUser(user) {
    if (!user) return null;
    return { id: user.id, username: user.username, email: user.email, role: user.role, createdAt: user.createdAt, fullName: user.fullName || '', phone: user.phone || '' };
  }
  ...
  var Auth = {
     ...
     */
    changePassword: function (userId, currentPassword, newPassword, confirmNewPassword) {
      ...
      if (newPassword !== confirmNewPassword) {
        return { success: false, error: 'Mật khẩu mới xác nhận không khớp.' };
      }
      ...
      if (user.passwordHash !== Utils.weakHash(currentPassword)) {
      ...
      user.passwordHash = Utils.weakHash(newPassword);
      saveUsers(users);
      return { success: true };
    },

    /**
     * Saves/updates the user's full name and phone — collected at checkout
     * time (not registration) so account creation stays quick, but
     * remembered afterward so returning buyers don't retype it.
     * @returns {{success:boolean, error?:string}}
     */
    updateContactInfo: function (userId, fullName, phone) {
      fullName = String(fullName || '').trim();
      phone = String(phone || '').trim();

      if (!userId) return { success: false, error: 'Bạn cần đăng nhập.' };
      if (!fullName) return { success: false, error: 'Vui lòng nhập họ và tên.' };
      if (!Utils.isValidPhone(phone)) return { success: false, error: 'Số điện thoại không hợp lệ.' };
